...
var _require2 = require('browser-sync'),
...
function buildStyles(cb) {
  gulp.src('./src/scss/**/*.scss').pipe(sass().on('error', sass.logError)).pipe(autoprefixer({
...
function buildStylesMinify(cb) {
  gulp.src('./public/css/*.css').pipe(sourcemaps.init()).pipe(cleanCSS()).pipe(sourcemaps.write()).pipe(gulp.dest("./public/css"));
  cb();
}
...
gulp.task('browser-sync', function (cb) {
  ...
  gulp.watch('./src/scss/**/*', gulp.series('sass')); // gulp.watch(jsWatch, bundleJS);
